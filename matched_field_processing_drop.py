...
if __name__ == '__main__':
    ...
    cloud_path = '/Users/jaredmagyar/Library/Mobile Documents/com~apple~CloudDocs/Outputs/Icequakes/sorsdal_catalogues'
    ...
    if all_events:
        for daychunk in chunk:
            ...
            filename = os.path.join(cloud_path,'mfp__diag__' + daychunk.str_name + '.csv')

            
            for event in tqdm.tqdm(day_cat,total=day_cat.N):

                event.attach_waveforms(ref_inv,ref_path,buffer=2,length=4,extra=10)
                #event.decimate(10)
                ...
                beampower_grid = event.vertical_csd_frq_loop(grid,frqlow=3,frqhigh=50,remove_response=True,normalise=False,frq_step=1,supress_noise=True)
    ...
    if group_events:
        ...
        for group, group_cat in temp_groups.items():
            if int(group) in [1,2,3,4,5,6,7,8]:
                ...
                filename = os.path.join(cloud_path,'mfp__diag__group__' + group + '.csv')
